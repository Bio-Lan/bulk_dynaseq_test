...
def get_all_bc(file,well_dict,well_split=False):
    ...
    for i in well_dict.keys():
        ...
        for j in well_dict[i]:
            ...
            all_bc[i]["sample"].update(func_protocol.get_mismatch_dict([barcodes[j-1]], 1))
            if well_split:
                all_bc[i]["well"][f'well{j}']=func_protocol.get_mismatch_dict([barcodes[j-1]], 1)
    return all_bc
...
class Split_Fastq:
    def __init__(self, args):
        ...
        if args.protocol == 'customized':
            ...
            self.pattern_dict = func_protocol.parse_pattern(pattern)
            self.whitelist_str = args.whitelist
        else:
            protocol = args.protocol
            protocol_dict = func_protocol.get_protocol_dict(args.assets_dir)
            if protocol == 'bulk_dynaseq-V1':
                if args.well == 96:
    ...
    def run(self):
        ...
        for i in range(self.fq1_number):
            with pysam.FastxFile(self.fq1_list[i], persist=False) as fq1,pysam.FastxFile(self.fq2_list[i], persist=False) as fq2:
                for entry1, entry2 in zip(fq1, fq2):
                    ...
                    seq_bc = seq1[self.pattern_dict["C"][0].start:self.pattern_dict["C"][0].stop]
                    for j in sub_bc.keys():
                        if seq_bc in sub_bc[j]["sample"].keys():
                            fh_fq1[j]["sample"].write(f'@{header1}\n{seq1}\n+\n{qual1}\n')
                            fh_fq2[j]["sample"].write(f'@{header2}\n{seq2}\n+\n{qual2}\n')
                            if self.args.split_to_well:
                                well_bc = sub_bc[j]["sample"][seq_bc]
                                well_num = sub_bc[j]["map"][well_bc]
        ...
        out_json = raw_sample + ".bulk_dynaseq.well_bc.json"
        utils.write_json(sub_bc,out_json)
        out_json = raw_sample + ".bulk_dynaseq.fastq_inf.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--protocol", required=True,default='bulk_dynaseq-V1')
