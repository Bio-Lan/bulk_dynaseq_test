...
nextflow run singleron-RD/bulk_rna \
 ...
 --outdir ./results \
 --run_splitfastq true \
 --sample_split 'path_to_split_information_file' \
 -profile docker
```
Option: 
``--well_split `True` ``
...
whith 'split_information_file' containg:
  | raw_sample | well | sub_sample |
  | ---------- | ----------|----------|
  |sample_x | 1-3,4,5 | sub_x |
  |sample_x | 11,12-15 | sub_y |
> [!NOTE]
> raw_sample is the same as sample in the samplesheet.
> well only allow `,` and `-`.Same well will make misktake.
